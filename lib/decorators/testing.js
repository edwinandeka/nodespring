...
function TestClass(testClass) {
  ...
  for (var _dataType in mocksToInject) {
    if (mocksToInject[_dataType] && !mocksToInject[_dataType].used) {
      _NodeSpringUtil2.default.error(_cliColor2.default.yellow.bold('  WARNING:'), _cliColor2.default.yellow('The declared mock for ' + testClass.name + '.' + mocksToInject[_dataType].testClassProperty + ' is not required on ' + objectToTest.instance.constructor.name + '\n'));
    }
  }
